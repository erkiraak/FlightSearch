...
def create_query_for_kiwi_api(search) -> dict:
    ...
    query = {
        'fly_from': search.fly_from.upper(),  # TODO create proper IATA search
        'fly_to': search.fly_to.upper(),  # TODO create proper IATA search
        'date_from': (search.departure_date - timedelta(days=search.flexible)).strftime("%d/%m/%Y"),
        'date_to': (search.departure_date + timedelta(days=search.flexible)).strftime("%d/%m/%Y"),
    ...
    if search.flight_type == "round":
        query['return_from'] = (search.return_date - timedelta(days=search.flexible)).strftime("%d/%m/%Y")
        query['return_to'] = (search.return_date + timedelta(days=search.flexible)).strftime("%d/%m/%Y")
